package bsadkhintest123ContigFilter::bsadkhintest123ContigFilterClient;
...
bsadkhintest123ContigFilter::bsadkhintest123ContigFilterClient
...
sub new
{
    ...
    my $self = {
	client => bsadkhintest123ContigFilter::bsadkhintest123ContigFilterClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a bsadkhintest123ContigFilter.FilterContigsParams
$output is a bsadkhintest123ContigFilter.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	assembly_input_ref has a value which is a bsadkhintest123ContigFilter.assembly_ref
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	assembly_output has a value which is a bsadkhintest123ContigFilter.assembly_ref
...
# Authentication: required
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "bsadkhintest123ContigFilter.filter_contigs",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub status
{
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
        method => "bsadkhintest123ContigFilter.status",
        params => \@args,
    });
    if ($result) {
        if ($result->is_error) {
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "bsadkhintest123ContigFilter.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for bsadkhintest123ContigFilter::bsadkhintest123ContigFilterClient\n";
    }
    if ($sMajor == 0) {
        warn "bsadkhintest123ContigFilter::bsadkhintest123ContigFilterClient version is $svr_version. API subject to change.\n";
    }
}
...
package bsadkhintest123ContigFilter::bsadkhintest123ContigFilterClient::RpcClient;
